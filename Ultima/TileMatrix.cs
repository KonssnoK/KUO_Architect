...
namespace Ultima
{
		...
		private unsafe HuedTile[][][] ReadStaticBlock( int x, int y )
		{
			...
			else
			{
				...
				fixed ( StaticTile *pTiles = staTiles )
				{
					...
					while ( pCur < pEnd )
					{
						lists[pCur->m_X & 0x7][pCur->m_Y & 0x7].Add( (short)((pCur->m_ID & 0x7FFF) + 0x8000), pCur->m_Hue, pCur->m_Z );
						++pCur;
					}
	...
	public struct Tile : IComparable
	{
		...
		public int CompareTo( object x )
		{
			...
			ItemData ourData = TileData.ItemTable[m_ID & 0x7FFF];
			ItemData theirData = TileData.ItemTable[a.m_ID & 0x7FFF];
